...
const BasicBarChartLog = ({width=960, height=650, margin}: BasicBarChartProps) => {
  ...
  useEffect(() => {
    ...
    select('.bar-chart-log .yAxis').call(axisLeft(yScaleRef.current).ticks(8))
    select('.bar-chart-log .xAxis').call(axisBottom(xScaleRef.current).ticks(parsedData.length-1).tickFormat(formatTime))
  },[bars, parsedData])
  return (
    <svg ref={svgRef} width={width} height={height} viewBox={`0 0 ${width} ${height}`} className='bar-chart-log'>
      {/* {parsedData.map(d => {
